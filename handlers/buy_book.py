...
PROCESS_BUY_BOOK, PROCESS_BUY_CATEGORY, PROCESS_BUY_THIS_BOOK, PROCESS_BUY_CUSTOMER_NAME, PROCESS_BUY_CUSTOMER_PHONE, PROCESS_BUY_CUSTOMER_ADDRESS, PROCESS_BUY_FINISH = range(7)
def fetch_books():
...
async def handle_buy_book(update: Update, context: CallbackContext) -> int:
    ...
    await query.message.reply_text('Select the category of the book you want to buy:', reply_markup=reply_markup)
    return PROCESS_BUY_CATEGORY

async def buy_process_category(update: Update, context: CallbackContext) -> int:
    print("<-------- arrived here <--------------")
    query = update.callback_query
    await query.answer()
    await query.message.delete()
    ...
    if genre != "all":
        filtered_books = [book for book in books if book['Genere'] == genre and book["Type"] == "Buy"]
    else:
        filtered_books = [book for book in books if book["Type"] == "Buy"]
    ...
    if not filtered_books:
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text="Select the book you want to buy:", reply_markup=menu)

    return PROCESS_BUY_BOOK

async def buy_process_book(update: Update, context: CallbackContext) -> int:
    query = update.callback_query
    await query.answer()
    ...
    keyboard = [
        [InlineKeyboardButton("Buy this book", callback_data=f'buy_this_book_{book_id}')],
        [InlineKeyboardButton("Back", callback_data='back')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(text=text, reply_markup=reply_markup)
    return PROCESS_BUY_THIS_BOOK

async def buy_this_book(update: Update, context: CallbackContext) -> int:
    query = update.callback_query
    await query.answer()
    ...
    return PROCESS_BUY_CUSTOMER_NAME

async def buy_customer_name(update: Update, context: CallbackContext) -> int:
    ...
    return PROCESS_BUY_CUSTOMER_PHONE

async def buy_customer_phone(update: Update, context: CallbackContext) -> int:
    context.user_data['buyer_phone'] = update.message.contact.phone_number
    await update.message.reply_text("Enter your block and room number:", reply_markup=ReplyKeyboardRemove())
    return PROCESS_BUY_CUSTOMER_ADDRESS

async def buy_customer_address(update: Update, context: CallbackContext) -> int:
    ...
    new_order = {
        ...
        'PhoneNumber': context.user_data['buyer_phone'],
        'Location': context.user_data['buyer_address'],
        'Type': 'buy',
